...
const routes = [
  ...
  },
  {
    path: "/contactService",
    name: "contactService",
    component: () => import("../views/ContactService/contactService.vue"),
    meta: {
      require: false,
      name: "contactService",
      title: "聯繫客服",
  ...
  },
  {
    path: "/question",
    name: "question",
    component: () => import("../views/Ｑuestion/question.vue"),
    meta: {
      require: false,
      name: "question",
      title: "常見問題",
...
router.beforeEach((to, from, next) => {
  ...
  if (to.meta.require) {
    api
      .GetLoginStatus()
      .then((data) => {
        router.app.$options.store.state.userToken = window.localStorage.getItem(
          "tokenClient"
        );
        router.app.$options.store.state.userName = window.localStorage.getItem(
          "userName"
        );
        next();
      })
